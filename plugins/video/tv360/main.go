...
const (
	defaultAPIURL   = "https://360zy.com/api.php/provide/vod"
	defaultSiteName = "360资源"
)
...
func (p *TV360Plugin) Fetch(req *sdk.FetchRequest) (*sdk.FeedResult, error) {
	...
	switch {
	case req.Route == "/tv360/list" || strings.HasPrefix(req.Route, "/tv360/list"):
		return fetchList(apiURL, siteName, req.ChannelID, req.Params)
	case req.Route == "/tv360/search" || strings.HasPrefix(req.Route, "/tv360/search"):
		query := strings.TrimSpace(req.Params["query"])
		if query == "" {
			return nil, fmt.Errorf("missing query parameter")
		}
		return fetchSearch(apiURL, siteName, query, req.Params)
	case req.Route == "/tv360/chapters/:id" || strings.HasPrefix(req.Route, "/tv360/chapters"):
		id := strings.TrimSpace(req.Params["id"])
		if id == "" {
			return nil, fmt.Errorf("missing id parameter")
		}
		return fetchChapters(apiURL, siteName, id)
	case req.Route == "/tv360/episode/:chapterId" || strings.HasPrefix(req.Route, "/tv360/episode"):
...
func fetchList(apiURL, siteName, channelID string, params map[string]string) (*sdk.FeedResult, error) {
	...
	result := &sdk.FeedResult{
		Title:       title,
		Description: fmt.Sprintf("第 %d 页，共 %d 条", page, int(resp.Total)),
		Items:       vodItemsToFeedItems(richItems),
	}
	if int(resp.PageCount) > page {
		result.HasMore = true
...
func buildEpisodePlayerHTML(item vodItem, episode playEntry, groups []playGroup, episodeIdx, activeSource int) string {
	...
	sb.WriteString(`<article class="tv360-player" style="color:#1f2937;line-height:1.6;">`)
	...
	if len(groups) > 1 {
		...
		for i, group := range groups {
			...
			sb.WriteString(fmt.Sprintf(
				`<button type="button" style="%s" onclick="tv360SwitchSource(%d)">%s</button>`,
	...
	sb.WriteString(`<video id="tv360-player" controls playsinline preload="metadata" style="width:100%;max-width:100%;border-radius:12px;background:#000;display:block;">`)
	sb.WriteString(`<source id="tv360-player-source" src="`)
	...
	if len(groups) > 1 {
		...
		sb.WriteString(`var tv360Sources=`)
		sb.WriteString(string(urlJSON))
		sb.WriteString(`;window.tv360SwitchSource=function(idx){`)
		sb.WriteString(`var url=tv360Sources[idx];if(!url){return;}`)
		sb.WriteString(`var player=document.getElementById("tv360-player");`)
		sb.WriteString(`if(!player){return;}`)
		sb.WriteString(`player.src=url;player.play();`)
		sb.WriteString(`var buttons=document.querySelectorAll(".tv360-player button");`)
...
func writeEpisodeInfoSection(sb *strings.Builder, item vodItem, episode playEntry, groups []playGroup, episodeIdx, activeSource int) {
	sb.WriteString(`<section class="tv360-info" style="margin-top:20px;padding-top:18px;border-top:1px solid #eef2f7;">`)
