...
const (
	...
	pageSize     = 30
	categoryID   = 1050
	platformPath = "disneyplus"
	pluginName   = "awwrated · Disney+"
)
...
func (p *Plugin) Fetch(req *sdk.FetchRequest) (*sdk.FeedResult, error) {
	switch {
	case req.Route == "/awwrated-disneyplus/list":
		return fetchList(req.ChannelID, req.Params)
	case req.Route == "/awwrated-disneyplus/search" || strings.HasPrefix(req.Route, "/awwrated-disneyplus/search"):
		query := strings.TrimSpace(req.Params["query"])
		if query == "" {
			return nil, fmt.Errorf("missing query parameter")
		}
		return fetchList(req.ChannelID, req.Params)
	case req.Route == "/awwrated-disneyplus/detail/:id":
