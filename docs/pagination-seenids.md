...
| 作用 | 告诉插件：「这些内容用户已经看过了，请不要再次返回」；是否继续传给上游由插件按源站行为决定 |
...
有些上游会提供「已曝光 ID 列表」字段，但这类字段不一定适合无限累计传入。插件应优先保证 `seenIds` 在 Orbit 会话内稳定去重；只有确认源站支持且不会因列表过长返回空结果时，才把它映射给上游。
...
- 第 2 页起：使用「加载更多」标记；`seenIds` 是否映射为上游曝光列表取决于源站稳定性
- 即梦当前实现不再传 `expose_item_id_list`，因为累计 `seenIds` 变长后，上游推荐流可能返回空列表；插件改为用 `offset/page` 拉取，再在本地过滤已见条目
